...
- `enquiry.html`, `enquiry.css`, `enquiry.js` and `enquiry-core.mjs` provide the public enquiry form. Its server endpoint fixes the two centre recipients, validates fields, and protects stored contacts. Never replace server-confirmed acceptance with a pretend-success message or a mailto-only submission.
  - **The enquiry server only answers the pages that serve the form** (`ORIGINS` in `enquiry-function/functions/service.js`, v1.415.1): `https://polymathlc.com.sg`, the `polymathlc/website` mirror's custom domain from its `CNAME` (preserved by the sync); `https://www.polymathlc.com.sg`; and `https://polymathlc.github.io`. A page on any other origin gets a 403 with no CORS headers, and the browser reports that to the form as a bare network failure. That is how the form broke on the custom domain while still working on github.io. **Moving the site means adding the new origin there and redeploying the `polymath-enquiry` codebase.**
  - **Redeploying is a GitHub Actions run, not a terminal** (`.github/workflows/deploy-enquiry.yml`, v1.415.2). It runs by itself when `enquiry-function/functions/**` or `enquiry-function/firebase.json` changes on main, and on demand from Actions → *Deploy enquiry service* → Run workflow. It needs two repository secrets, `FIREBASE_SERVICE_ACCOUNT` and `POLYMATH_ENQUIRY_RECIPIENTS` (see `enquiry-function/README.md`). It deploys `--only functions:polymath-enquiry` and nothing else: never `--force`, never Firestore or Storage rules (they are shared by every app). The narrow enquiry-mail rule is installed by `tools/enquiry-rules.mjs` only on a manual run with its box ticked. `tools/enquiry-smoke.mjs` then sends CORS preflights, never a POST (a POST emails the centre), and fails the run unless every origin in `ORIGINS` is accepted and an unlisted one is refused. `tools/enquiry-deploy-setup.mjs` turns the secrets into the key file and `.env.mathgen--app`, and never prints either: a run's log is readable by everyone who can see the repository.
  - **Do not copy the September registration page's approach** (the browser writing `mail` and registration documents directly), however much simpler it looks. The shared Firestore rules let anyone on the internet create any `mail` document, so that approach turns the centre's mail account into an open relay. It also publishes the recipient addresses in page code, and stored contacts become readable by every visitor. The server exists to fix the recipients, validate, rate-limit and keep contacts private.
  - **Only a browser that reports itself offline is told to check its connection** (`failureMessage` in `enquiry-core.mjs`). A refused, down or blocked service looks identical to a dead connection from `fetch()`. So it is reported as `network` ("We couldn't reach our enquiry service") and offers `DIRECT_CONTACT`: the WhatsApp line the home page already publishes, which the harness keeps in step. The link is shown beside a failure, never instead of the server's confirmation.
  - Run `npm test --prefix enquiry-function/functions`, `node tools/enquiry-tests.mjs` and `node tools/enquiry-deploy-tests.mjs` after touching any of it.
